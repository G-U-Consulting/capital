...
select c.nombres as `firstName`, concat(coalesce(c.apellido1, ''), ' ', concat(coalesce(c.apellido2, ''))) as `lastName`, c.nombre_empresa as `company`,
    ...
    if(c.telefono1 is not null, c.codigo_tel1, c.codigo_tel2) as `indicative`, coalesce(c.email1, c.email2) as `email`, c.ciudad as `cityOrigin`,
...
/*
select * from fact_visitas ORDER BY created_on desc;
select * from fact_clientes ORDER BY created_on desc;
*/