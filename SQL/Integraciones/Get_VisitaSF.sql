...
select c.nombres as `firstName`, concat(coalesce(c.apellido1, ''), concat(coalesce(c.apellido2, ''))) as `lastName`, null as `company`,
    c.numero_documento as `document`, c.tipo_documento as `typeDoc`, s.sede as `cityLead`, c.pais_expedicion as `countryExpedition`,
    ...
    cast(coalesce(c.telefono1, c.telefono2) as unsigned) as `mobilePhone`, date_format(c.fecha_nacimiento, '%Y-%m-%d') as `birthDate`, c.ciudad as `cityResidence`, 
    c.departamento as `department`, c.direccion as `direction`, c.pais as `countryResidence`, c.is_politica_aceptada as `_AuthorizeData`, v.id_visita,
    p.rango as `disposeToInvest`, t.tipo_registro as `registerType`, ma.modo_atencion as `attentionReason`, c.id_cliente as `IdClient`, m.motivo_compra as `reasonPurchase`,
    true as `_visitedSalesRoom`, sv.sala_venta as `salesRoom`, date_format(v.created_on, '%Y-%m-%d') as `visitedDate`
from fact_visitas v
