...
on p.id_proyecto = sp.id_proyecto join dim_personal_sala ps
on sp.id_sala_venta = ps.id_sala_venta join fact_usuarios u
...
select h.id_hito, h.titulo, h.descripcion, date_format(h.fecha, '%Y-%m-%d %T') as fecha, h.color, h.festivo, h.id_sala_venta, 
    h.id_proyecto, h.frecuencia, date_format(h.limite, '%Y-%m-%d') as limite, sv.sala_venta, pro.nombre as nombre_pro,
...
select ps.fecha, ps.id_sala_venta, sv.sala_venta, ps.id_estado, e.estado
