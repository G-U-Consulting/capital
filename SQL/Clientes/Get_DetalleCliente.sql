...
select date_format(l.created_on, '%Y-%m-%d') as created_on, l.*, coalesce(c.email1, email2) as email, coalesce(c.telefono1, c.telefono2) as telefono, 
    c.numero_documento, concat(coalesce(c.nombres, ''), ' ', coalesce(c.apellido1, ''), ' ', coalesce(c.apellido2, '')) as nombre_cliente,
...
join fact_negocios_unidades cu on o.id_cotizacion = cu.id_cotizacion
join fact_unidades un on cu.id_unidad = un.id_unidad
