export default {
    ...
    methods: {
        ...
        async onCreateDocument() {
            let doc = this.documento;
            try {
                showProgress();
                const resp = await httpFunc(`/generic/genericST/Maestros:Ins_Documento`, doc, true);
                hideProgress();
                if (resp.data === "OK" && resp.id) {
                    doc.id_documento = resp.id;
        ...
        getIcon(ext) {
            ...
            if (['xls', 'xlsx', 'xlsm', 'xlt', 'xltx', 'xltm'].includes(ext)) return base + 'Excel.png';
